...
def infer_gnn(tr_data, val_data, te_data, tr_inds, val_inds, te_inds, args, data_config):
    ...
    wandb.init(
        ...
        config={
            ...
            "loss": extract_param("loss", args),        # <-- Reads "focal" now
            "alpha": extract_param("alpha", args),      # <-- Added
            "gamma": extract_param("gamma", args),      # <-- Added
            # "w_ce1": extract_param("w_ce1", args),    # <-- Removed
            # "w_ce2": extract_param("w_ce2", args),    # <-- Removed
